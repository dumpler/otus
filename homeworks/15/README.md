...
## Изменения в исходной DDL

В задании связь продажи с магазином была записана прямо в описании колонки:

```sql
store_id BIGINT REFERENCES stores (store_id)
```

Для MySQL 8.0.15 такой синтаксис не создает внешний ключ в итоговой таблице

Поэтому связь описана явно через `CONSTRAINT`
...
Так в `sales` действительно появляется внешний ключ на `stores`
